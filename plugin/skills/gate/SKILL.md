...
The command below asked smallprint.dev, for each MCP server this machine has installed, whether its small print changed on the record since the lock and whether a high or critical advisory names it. It sent the names and versions of the servers and nothing else. It exits with code 2 when a small print changed and 3 when a high advisory names a server.

```!
npx -y smallprint@0.1.4 gate || true
```
