...
`check` also hashes your instruction files (CLAUDE.md and what it imports, AGENTS.md, an OpenClaw workspace's TOOLS.md and SOUL.md, Cursor, Windsurf, Copilot, Gemini, Cline and Roo rules, Claude agents, commands and settings with hooks, and the server definitions in your MCP configs with env values hashed) and compares them with the last run on the same machine: first seen, unchanged since a date, changed, or removed. A prompt injection that rewrites one of these persists into every later run; this is the line that says so. For settings files the line names the section that changed (`in: permissions` is an always-allow click, `in: hooks` is worth reading). Three ways to keep that record, weakest to strongest, your choice:
...
```sh
npx smallprint gate --project   # exit 2 when a server's small print changed on the record since the lock, 3 when a high advisory names one
